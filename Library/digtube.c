...
void DigTube_Display()
{
	...
	for(i=0;i<4;i++)
	{
		switch(i)	 //位选，选择点亮的数码管，
		{
			...
			case(0):
				DIGTUBE_CON= control_code[2]; break;//显示第4位
			case(1):
				DIGTUBE_CON= control_code[3];break;//显示第3位
			case(2):
				DIGTUBE_CON= control_code[1];break;//显示第5位
			case(3):
				DIGTUBE_CON= control_code[0];break;//显示第6位
		}
